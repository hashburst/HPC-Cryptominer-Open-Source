...
# HPC Cryptominer Entrypoint Script for Podman (RHEL 10)
set -e

echo "Starting HPC Cryptominer..."
echo "Container: Podman (RHEL 10)"
echo "User: $(whoami)"
...
asyncio.run(detect())
" || echo "Hardware detection failed, continuing..."
...
if [ ! -f "/app/config/mining_config.json" ]; then
    ...
    python3 /app/create_default_config.py || echo "Failed to create config, using defaults"
fi
